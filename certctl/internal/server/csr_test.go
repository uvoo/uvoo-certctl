...
func TestCSRSubmitAndPickupFlow(t *testing.T) {
	...
	_ = writer.WriteField("kind", storage.CertKindPublic)
	_ = writer.WriteField("submit_password", "submit-secret")
	_ = writer.WriteField("requester_name", "Jane Doe")
	part, err := writer.CreateFormFile("csr", "server.csr")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := part.Write(mustCreateServerTestCSR(t, "api.example.com", []string{"api.example.com"})); err != nil {
		t.Fatal(err)
	}
