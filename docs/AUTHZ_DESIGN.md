...
This document defines a minimal authentication and authorization design for `uvoo-certctl`'s built-in HTTP(S) server.
...
This is why `uvoo-certctl` should not hardcode a single roles claim name.
...
If `uvoo-certctl` stored provider certs statically, operators would have to keep them current manually. That is more fragile and usually more code overall.
...
For `uvoo-certctl`, logical names and common names are usually more stable than row UUIDs, so they are the better first scope key.
...
- `realm_access.roles`
- `resource_access.uvoo-certctl.roles`
...
`uvoo-certctl` does not need SQL session auth context to make route-level decisions, so the simpler route middleware approach is a better fit here.
...
For local testing, a small Docker Compose stack with Keycloak and `uvoo-certctl` is worthwhile.
...
That gives you Azure, AWS/Cognito, Google, Keycloak, and similar providers with one implementation path and without turning `uvoo-certctl` into an auth framework.
